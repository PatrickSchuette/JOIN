...
function showEditDialogMobile() {
  ...
  document.getElementById("img_edit_mobile").src =
    "../assets/img/edit_contact.svg";
...
function deleteContactMobileView() {
  let mail = document.getElementById("mail_data").innerText;
  document.getElementById("img_delete_mobile").src =
    "../assets/img/delete_contact.svg";
  deleteContact(mail);
}
...
/**
 * Adds backdrop-click listeners to all dialogs so they close when the user clicks outside the content.
 */
function initDialogBackdropClose() {
  const addContactDialog = document.getElementById("add_contact_dialog");
  const editContactDialog = document.getElementById("edit_contact_dialog");
  const msgContactDialog = document.getElementById("msg_contact_dialog");

  if (addContactDialog) {
    addContactDialog.addEventListener("click", (event) => {if (event.target === addContactDialog) {closeDialogAddContact();}});
  }

  if (editContactDialog) {
    editContactDialog.addEventListener("click", (event) => {if (event.target === editContactDialog) {closeDialogEditContact();}});
  }
